...
// PATCH /api/notes/[id] — update title, content, summary, categoryIds, createdAt, pinned, archived
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    // Handle categoryIds — full replace when provided
    if (Array.isArray(body.categoryIds)) {
      ...
      const note = await prisma.$transaction(async (tx) => {
        await tx.noteCategory.deleteMany({ where: { noteId: params.id } })
        if (categoryIds.length > 0) {
          await tx.noteCategory.createMany({
            data: categoryIds.map((cid: string) => ({ noteId: params.id, categoryId: cid })),
          })
        }
