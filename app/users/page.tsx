...
export default function UsersPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      {/* Nav */}
      <nav className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-slate-200/60 safe-top">
        <div className="max-w-lg mx-auto px-5 h-14 flex items-center gap-3">
          ...
          <h1 className="text-sm font-bold text-slate-900">Kelola Pengguna</h1>
        </div>
      </nav>
      ...
      <div className="max-w-lg mx-auto px-4 py-5 space-y-5">
        <div className="pt-1 pb-1">
          ...
          <p className="text-sm text-slate-500 mt-1">Tambah, ubah password, atau hapus pengguna.</p>
        </div>

        {/* User list */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100">
            <p className="text-sm font-semibold text-slate-900">Daftar Pengguna</p>
          </div>
          {loading ? (
            <div className="flex justify-center py-10"><LoadingSpinner size="md" className="border-slate-200 border-t-sky-500" /></div>
          ) : error ? (
                ...
                <li key={user.id} className="flex items-center justify-between px-5 py-3.5">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md uppercase tracking-wide ${user.role === 'admin' ? 'bg-sky-100 text-sky-700' : 'bg-slate-100 text-slate-500'}`}>
                        {user.role}
                      </span>
                      {me?.userId === user.id && (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-emerald-100 text-emerald-700 uppercase tracking-wide">Saya</span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 ml-3">
                    <button type="button"
                      ...
                      className="px-2.5 py-1.5 text-xs font-medium rounded-lg text-slate-500 hover:text-sky-600 hover:bg-sky-50 transition-colors">
                      Ubah Password
                    </button>
        ...
        {me?.role === 'admin' && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100">
              <p className="text-sm font-semibold text-slate-900">Tambah Pengguna Baru</p>
            </div>
            <form onSubmit={handleAdd} className="px-5 py-4 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <input type="text" value={newUsername} onChange={e => { setNewUsername(e.target.value); setAddError(null) }}
                    ...
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-sky-500/40 focus:border-sky-400 transition-colors" />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide">Role</label>
                <div className="flex gap-2">
                  {(['user', 'admin'] as const).map(r => (
                    <button key={r} type="button" onClick={() => setNewRole(r)}
                      className={`flex-1 py-2 rounded-xl text-xs font-semibold border capitalize transition-colors ${newRole === r ? 'bg-sky-500 border-sky-500 text-white' : 'bg-slate-50 border-slate-200 text-slate-500 hover:border-sky-300'}`}>
                      {r}
                    </button>
                  ))}
                </div>
              </div>
              {addError && <p className="text-xs text-red-500 bg-red-50 border border-red-100 rounded-xl px-3 py-2">{addError}</p>}
      ...
      {changePw && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full sm:max-w-sm bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl p-6 z-10">
            <h3 className="text-base font-bold text-slate-900 mb-1">Ubah Password</h3>
            <p className="text-sm text-slate-500 mb-4">Pengguna: <span className="font-medium text-slate-700">{changePw.username}</span></p>
            <form onSubmit={handleChangePassword} className="space-y-4">
              <div className="space-y-1.5">
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide">Password Baru</label>
                <input type="password" value={changePw.password}
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-900 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-sky-500/40 focus:border-sky-400 transition-colors" />
              </div>
              {changePw.error && <p className="text-xs text-red-500 bg-red-50 border border-red-100 rounded-xl px-3 py-2">{changePw.error}</p>}
              <div className="flex gap-2">
                <button type="button" onClick={() => setChangePw(null)}
                  className="flex-1 h-10 rounded-xl border border-slate-200 text-sm text-slate-500 hover:bg-slate-50 transition-colors">Batal</button>
