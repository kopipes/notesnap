    ...
    <div className="flex flex-col min-h-full">
      ...
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-slate-200/60 safe-top">
        <div className="max-w-lg mx-auto px-5 h-14 flex items-center">
          <h1 className="text-sm font-bold text-slate-900">Kalender</h1>
        </div>
      </header>
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isSelected ? 'bg-sky-500 text-white' : isToday ? 'bg-sky-50 text-sky-600' : 'text-slate-700 hover:bg-slate-50'}
                  `}
                >
                  {day}
                  {dot && (
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-slate-200 px-5 py-8 text-center">
              <p className="text-slate-400 text-sm">Tidak ada catatan pada hari ini</p>
            </div>
          ) : (
            <div className="space-y-2">
              {notes.map(note => (
                <button
                  ...
                  className="w-full text-left bg-white rounded-2xl border border-slate-200 px-4 py-3.5 shadow-sm hover:border-sky-300 hover:shadow-md transition-all active:scale-[0.98] flex items-center justify-between"
                >
                  <div>
                    <p className="text-sm font-semibold text-slate-900 truncate">{note.title || 'Catatan Tanpa Judul'}</p>
                    <p className="text-xs text-slate-400 mt-0.5">
                      {new Date(note.createdAt).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-slate-300 shrink-0 ml-3">
                    <path fillRule="evenodd" d="M16.28 11.47a.75.75 0 010 1.06l-7.5 7.5a.75.75 0 01-1.06-1.06L14.69 12 7.72 5.03a.75.75 0 011.06-1.06l7.5 7.5z" clipRule="evenodd" />
                  </svg>
                </button>
              ))}
